...
- /api
  ...
  - /usercar (차량 및 주차 관리)
    - GET /
      - 인원간 차량 조회
      - querystring {("limit":string , "offset":string) }
      - return Array< ICarInfo = {id : int ,kind :string , carcode : string ,createdAt : string ,agreementCount : int , oppositition : int , users.name : string , group(group.Name) : string(Name) , groupByGroup(groupByGroup.name) : string }>
    - GET /filter
